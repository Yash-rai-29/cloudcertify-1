...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    startLoading();
    try {
      const [userResponse, streakResponse, activitiesResponse] = 
        await Promise.all([
          getUserInfo(),
          getDailyStreak(),
          getUserActivities()
      ...
      // Recommendations will be fetched separately based on activeTab
    } catch (error) {
  ...
  // Fetch recommendations based on selected tab
  const fetchRecommendations = async (tabType) => {
    try {
      const recommendationsResponse = await getTestRecommendations(10, tabType);
      if (recommendationsResponse.success) {
        setRecommendations(recommendationsResponse.data.recommendations || []);
      }
    } catch (error) {
      console.error('Error fetching recommendations:', error);
      showError('Failed to load test recommendations. Please try again.');
    }
  };
        ...
        >
          ...
          >
            ...
            <DashboardCard
              ...
              subtitle={`${userData?.activity?.tests_passed || 0} passed`}
            />
          </motion.div>
            ...
            <div className="lg:col-span-2">
              <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="flex border-b border-gray-200 mb-4">
                  ...
                  <button
                    className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                      activeTab === 'in-progress' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'
                    }`}
                  ...
                  <button
                    className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${
                      activeTab === 'personalized' ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'
                    }`}
